...
1. **모노톤 + 한 점의 강조색** — 기본은 검정~회색 램프 하나다. 유채색은 표지 부케(`couple.svg`)에서 가져온 `--accent` **하나뿐**이고, **예식일(11월 28일) 표시에만** 쓴다. 그 외의 강조는 색이 아니라 **무게(weight)와 여백**으로 만든다.
...
| `--accent` | `#ffef50` | **유일한 유채색** — 달력의 예식일 원. 표지 부케에서 가져왔다 |

**금지:** `--accent` 외의 유채색 추가. 그림자(`box-shadow`). 위 램프에 없는 회색 값.

`--accent`를 다른 곳(버튼·헤딩·링크 등)으로 넓히지 않는다. **한 곳에만 쓰기 때문에** 강조로 작동한다.
...
일요일 열과 지난 날짜는 `--ink-ghost`. 예식일은 `700` + `::before` 30px 원을 `--accent`로 채우고 숫자는 `--ink`로 둔다.
원은 `z-index: -1`로 숫자 뒤에 깔리며, 부모의 `isolation: isolate`가 원이 셀 밖 스택으로 빠지는 것을 막는다.
